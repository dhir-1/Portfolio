...
const ALL_PROJECTS = [
  ...
  },
  {
    ...
    longBody: "Engineered to eliminate the critical failure modes of naive RAG in high-stakes financial analysis: blind retrieval contamination, entity starvation in comparisons, and ungrounded citations. Orchestrated as a LangGraph StateGraph, the architecture integrates a dynamic zero-hardcoding filing registry that enforces a 50/50 balanced retrieval quota for cross-company queries (e.g. AMD vs NVIDIA). Replaces slow LLM self-reflection with a CPU-native FlashRank cross-encoder confidence gate (0.001 logit threshold), and enforces schema-bounded synthesis requiring valid SEC citation enums [TICKER, Section]. Deterministic pre-flight guardrails intercept out-of-scope entities in under 5ms at 0 tokens and $0 cost, while post-flight regex audits all financial numbers and dates against filing chunks.",
